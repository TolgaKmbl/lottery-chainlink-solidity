...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    function getEntranceFee() public view returns(uint256){
        ...
        uint256 price = getLatestEthUsdPrice(); // 8 decimals FIAT
    ...
    function startLottery() public onlyOwner {
        require(lotteryState == LOTTERY_STATE.CLOSED, "Cannot start a new lottery now");
        lotteryState = LOTTERY_STATE.OPEN;
        randomResult = 0;
    }

    function endLottery() public onlyOwner {
        require(lotteryState == LOTTERY_STATE.OPEN, "Cannot end the lottery yet");
        lotteryState = LOTTERY_STATE.CALCULATING_WINNER;
        pickWinner();
    }

    function pickWinner() private returns(bytes32) {
        require(lotteryState == LOTTERY_STATE.CALCULATING_WINNER, "Must be calculating the winner");
        bytes32 requestId = requestRandomness(keyHash, fee);
        emit RequestedRandomness(requestId);
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        require(randomness > 0, "Not a random number");
